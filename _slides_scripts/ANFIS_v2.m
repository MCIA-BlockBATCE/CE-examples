...
% train/test split
IO_matrix_training = IO_matrix(51:600,:);
IO_matrix_testing = IO_matrix(601:length(data),:);

% ANFIS
opt=anfisOptions('EpochNumber',10);
fis = anfis(IO_matrix_training,opt);
predicted_data = evalfis(fis,IO_matrix_testing(:,1:7));
RMS_1h = rms(predicted_data' - data(1,601:length(data)));
...
figure(1)
plot(1:length(predicted_data),predicted_data',1:length(predicted_data),data(1,601:length(data)))
